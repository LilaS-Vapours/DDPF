...
from .decoder_experiment import Aux_Module
import numpy as np
...
class ModelBuilder(nn.Module):
    ...
    def forward(self, x, need_fp=False, use_corr=False,flag_use_fdrop = False):
        h, w = x.shape[-2:]
        if self._use_auxloss:
            ...
            if flag_use_fdrop:
                ...
                feat = self.encoder(x)
                if need_fp:
                    outs, corr_out = self.decoder(feat, True)
                    corr_out = F.interpolate(corr_out, (h, w), mode="bilinear", align_corners=True)
                    outs = F.interpolate(outs, (h, w), mode="bilinear", align_corners=True)
                    return outs, corr_out
                if use_corr:
                    outs, corr_out = self.decoder(feat, False, True)
                    corr_out = F.interpolate(corr_out, (h, w), mode="bilinear", align_corners=True)
                    outs = F.interpolate(outs, (h, w), mode="bilinear", align_corners=True)
                    return outs, corr_out
