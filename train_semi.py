...
from augseg.utils.loss_helper import get_criterion, compute_unsupervised_loss_by_threshold, \
    compute_unsupervised_loss_by_threshold_kl
...
def main(in_args):
    ...
    if rank == 0:
        ...
        csv_path = os.path.join('/home/ubuntu/log', "seg_{}_stat.csv".format(curr_timestr))
        # data_frame.to_csv('/home/ubuntu/seg_2024-01-12_18:26:18_stat.csv', index_label='epoch', mode='w')
    ...
    unsup_loss_fn = get_criterion(cfg, True)
    ...
    for epoch in range(last_epoch, cfg_trainer["epochs"]):
        ...
        if rank == 0:
            ...
            logger.info(
                " <<Test>> - Epoch: {}.  MIoU: {:.2f}/{:.2f}.  \033[34mBest-STU:{:.2f}/{}  \033[31mBest-EMA: {:.2f}/{}\033[0m".format(
                    epoch,
...
def train(
    ...
    loader_l_iter = iter(loader_l)  # 有标签数据
    loader_u_iter = iter(loader_u)  # 无标签数据
    assert len(loader_l) == len(loader_u), f"labeled data {len(loader_l)} unlabeled data {len(loader_u)}, mixmatch!"

    # metric indicators
    sup_losses = AverageMeter(20)  # 有监督损失
    uns_losses = AverageMeter(20)  # 无监督损失
    batch_times = AverageMeter(20)  # 批次训练时间
    learning_rates = AverageMeter(20)  # 学习率
    meter_high_pseudo_ratio = AverageMeter(20)  # 高置信度伪标签比例
    ...
    for step in range(len(loader_l)):
        ...
        _, image_l, label_l = loader_l_iter.next()  # 获取有标签数据和标签        (index,image,label)
        image_l, label_l = image_l.cuda(), label_l.cuda()
        _, image_u_weak, image_u_aug, _ = loader_u_iter.next()  # 获取弱增强和强增强的无标签数据   index, image_weak, image_strong, label
        image_u_weak, image_u_aug = image_u_weak.cuda(), image_u_aug.cuda()

        num_labeled = len(image_l)
        num_unlabeled = len(image_u_aug)
        # start the training
        if epoch < cfg["trainer"].get("sup_only_epoch", 0):
            ...
            with torch.no_grad():
                model_teacher.eval()
                pred_u = model_teacher(image_u_weak.detach())
                # print(pred_u.size())
            ...
            if np.random.uniform(0, 1) < trigger_prob and cfg["trainer"]["unsupervised"].get("use_cutmix",
                                                                                             False):  # 有设置就读取，没有就默认为False
            ...
            if flag_extra_weak:  # 对有标签数据进行额外的弱增强
                pass



            pred_all, preds_corr = model(torch.cat((image_l, image_u_weak), dim=0), True)
            pred_u_strong, preds_corr_strong = model(image_u_aug,False,True)
            aux_all = None
            # print('pred_all is', pred_all.size())
            del image_l, image_u_aug

            pred, pred_fp = pred_all.chunk(2)
            pred_l = pred[:num_labeled]
            pred_weak = pred[num_labeled:]
            pred_weak_fp = pred_fp[num_labeled:]

            softmax_pred_u_aug = F.softmax(pred_weak.detach(), dim=1)
            # print(softmax_pred_u_aug.size())
            # print('pred_l is', pred_l.size())
            # print('label_l is', label_l.size())
            logsoftman_pred_u_aug = F.log_softmax(pred_u_strong.detach(), dim=1)
            logits_u_w, label_u_w = torch.max(softmax_pred_u_aug, dim=1)

            # pred_x_corr = preds_corr[:num_labeled]
            pred_u_w_corr = preds_corr[num_labeled:]
            label_u_w_corr = label_u_w.clone()
            label_u_strong_corr = label_u_aug.clone()

            # print('pred_u_strong is', pred_u_strong.size())
            # print('label_u_aug is ', label_u_aug.size())
            del pred_all, preds_corr
            ...
            # 5. unsupervised loss
            unsup_loss_u_strong, pseduo_high_ratio = compute_unsupervised_loss_by_threshold(
                pred_u_strong, label_u_aug.detach(),
                logits_u_aug.detach(), thresh=p_threshold)
            unsup_loss_u_strong *= cfg["trainer"]["unsupervised"].get("loss_weight", 1.0)

            unsup_loss_u_strong_corr, _ = compute_unsupervised_loss_by_threshold(
                preds_corr_strong, label_u_strong_corr.detach(),
                logits_u_aug.detach(), thresh=p_threshold)
            unsup_loss_u_strong_corr *= cfg["trainer"]["unsupervised"].get("loss_weight", 1.0)

            unsup_loss_u_weak_corr, _ = compute_unsupervised_loss_by_threshold(
                pred_u_w_corr, label_u_w_corr.detach(),
                logits_u_w.detach(), thresh=p_threshold)
            unsup_loss_u_weak_corr *= cfg["trainer"]["unsupervised"].get("loss_weight", 1.0)

            unsup_loss_corr = 0.5 * (unsup_loss_u_weak_corr + unsup_loss_u_strong_corr)

            loss_u_kl = compute_unsupervised_loss_by_threshold_kl(
                logsoftman_pred_u_aug, softmax_pred_u_aug.detach(),
            )

            conf_u = F.softmax(pred_weak.detach(),dim=1)
            conf_u = torch.max(conf_u,dim=1)[0]
            conf_fliter_u = ((conf_u >= p_threshold)&(label_u_aug != 255))
            loss_u_kl = torch.sum(loss_u_kl,dim=1) * conf_fliter_u
            loss_u_kl = torch.sum(loss_u_kl) / torch.sum(label_u_aug != 255).item()
            loss_u_kl *= cfg["trainer"]["unsupervised"].get("loss_weight", 1.0)

            unsup_loss_u_weak_fp, _ = compute_unsupervised_loss_by_threshold(
                pred_weak_fp, label_u_w.detach(),
                logits_u_w.detach(), thresh=p_threshold)
            unsup_loss_u_weak_fp *= cfg["trainer"]["unsupervised"].get("loss_weight", 1.0)


            unsup_loss = 0.25 * unsup_loss_u_strong + 0.25 * unsup_loss_corr + 0.25 * unsup_loss_u_weak_fp + 0.25 * loss_u_kl

            del pred_l, pred_u_strong, label_u_aug, logits_u_aug, preds_corr_strong,pred_weak_fp
